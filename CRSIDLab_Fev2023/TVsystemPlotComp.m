function TVsystemPlotComp(pHandle,pCurVar,userOpt,tag)
...
if ~isempty(options.session.tvsys.sigLen)
    ...
    opt = get(pHandle,'tag');
    if strcmp(opt,'out'), type = options.session.tvsys.sigSpec{1};
    elseif strcmp(opt,'in1'), type = options.session.tvsys.sigSpec{2};
    elseif strcmp(opt,'in2'), type = options.session.tvsys.sigSpec{3};
    end
    ...
    elseif strcmp(tag,'detrend')
        dataDet = identDetrend(data,curVar.fs,options.session.tvsys.perc,...
            options.session.tvsys.poly);
        data = data - mean(data);
        tam = round((options.session.tvsys.perc/100)*length(data));
    ...
    pos = time(round((options.session.tvsys.perc/100)*length(data)));
