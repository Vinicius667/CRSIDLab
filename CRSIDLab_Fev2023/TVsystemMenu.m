...
function TVsystemMenu(pnSystem,pFile,userOpt)
% SystemMenu - CRSIDLab
...
puVar.in2 = uicontrol('parent',pnSystem,'style','popupmenu','tag','in2',...
    ...
    'value',1,'userData',1,'units','normalized','position',...
    [.67 .93 .2 .04],'backgroundColor',[1 1 1],'Enable','off');
...
aux = options.session.tvsys.sigSpec(~cellfun(@isempty,...
    options.session.tvsys.sigSpec));
if length(aux) == 3
    ...
    if options.session.tvsys.viewFilter
        TVsystemPlotComp(pHandle.triple1,pCurVar,userOpt,'filt');
        TVsystemPlotComp(pHandle.triple2,pCurVar,userOpt,'filt');
        TVsystemPlotComp(pHandle.triple3,pCurVar,userOpt,'filt');
    elseif options.session.tvsys.viewDetrend
        TVsystemPlotComp(pHandle.triple1,pCurVar,userOpt,'detrend');
        TVsystemPlotComp(pHandle.triple2,pCurVar,userOpt,'detrend');
        TVsystemPlotComp(pHandle.triple3,pCurVar,userOpt,'detrend');
    else
        TVsystemPlot(pHandle.triple1,pCurVar,userOpt);
        TVsystemPlot(pHandle.triple2,pCurVar,userOpt);
        TVsystemPlot(pHandle.triple3,pCurVar,userOpt);
    end
elseif length(aux) == 2
    set(pHandle.single,'visible','off');
    set(get(pHandle.single,'children'),'visible','off');
    ...
    if options.session.tvsys.viewFilter
        TVsystemPlotComp(pHandle.double1,pCurVar,userOpt,'filt');
        TVsystemPlotComp(pHandle.double2,pCurVar,userOpt,'filt');
    elseif options.session.tvsys.viewDetrend
        TVsystemPlotComp(pHandle.double1,pCurVar,userOpt,'detrend');
        TVsystemPlotComp(pHandle.double2,pCurVar,userOpt,'detrend');
    else
        TVsystemPlot(pHandle.double1,pCurVar,userOpt);
        TVsystemPlot(pHandle.double2,pCurVar,userOpt);
    end
else
    set(pHandle.double1,'visible','off');
    set(get(pHandle.double1,'children'),'visible','off');
    ...
    if length(aux) == 1
        if options.session.tvsys.viewFilter
            TVsystemPlotComp(pHandle.single,pCurVar,userOpt,'filt');
        elseif options.session.tvsys.viewDetrend
            TVsystemPlotComp(pHandle.single,pCurVar,userOpt,'detrend');
        else
            TVsystemPlot(pHandle.single,pCurVar,userOpt);
        end
...
for i = 1:3
    ...
    auxString = stringPU;
    if ~isempty(options.session.tvsys.varString.(puOpt{i}))
        for j = 1:3
            if j ~= i && options.session.tvsys.varValue.(puOpt{j})~=1
                prevVar = options.session.tvsys.varString.(puOpt{j}){...
                    options.session.tvsys.varValue.(puOpt{j})};
    ...
    if ~isequal(options.session.tvsys.varString.(puOpt{i})(2:end),auxString)
        if ~isempty(options.session.tvsys.varString.(puOpt{i}))
            if ismember(options.session.tvsys.varString.(puOpt{i}){...
                options.session.tvsys.varValue.(puOpt{i})},auxString)
                options.session.tvsys.varValue.(puOpt{i}) = find(ismember(...
                    auxString,options.session.tvsys.varString.(puOpt{i}){...
                    options.session.tvsys.varValue.(puOpt{i})}))+1;
            else
                options.session.tvsys.varValue.(puOpt{i}) = 1;
            end
        end
        options.session.tvsys.varString.(puOpt{i}) = [puID(i);auxString];
    end
    
    % setup popupmenus
    set(puVar.(puOpt{i}),'string',...
        options.session.tvsys.varString.(puOpt{i}));
    set(puVar.(puOpt{i}),'value',...
        options.session.tvsys.varValue.(puOpt{i}));
...
options.session.tvsys.perc = checkLim(options.session.tvsys.perc,'perc');
set(tePerc,'string',num2str(options.session.tvsys.perc));
options.session.tvsys.poly = checkLim(options.session.tvsys.poly,'poly');
set(tePoly,'string',num2str(options.session.tvsys.poly));
...
for i = 1:3
    auxVar = options.session.tvsys.varString.(opt{i}){...
        options.session.tvsys.varValue.(opt{i})};
...
options.session.tvsys.sigSpec = type;
for i = 1:length(type)
    if strcmp(type{i},'hr')
        type{i} = 'rri';
    end
...
if ~isempty(type{1}) || ~isempty(type{2}) || ~isempty(type{3})
    ...
    for i = 1:3
        if ~isempty(type{i})
            ...
            if isempty(options.session.tvsys.sigLen)
                options.session.tvsys.sigLen = length(curVar.(opt{i}));
            end
        end
    end
end
...
if oldValue ~= newValue
    ...
    if (strcmp(get(scr,'tag'),'in2') && ...
            options.session.tvsys.varValue.in1 == 1) || ...
            (strcmp(get(scr,'tag'),'in1') && ...
            options.session.tvsys.varValue.out == 1)
    ...
    elseif (strcmp(get(scr,'tag'),'in1') && newValue == 1 && ...
            options.session.tvsys.varValue.in2 ~= 1) || ...
        ...
        errStat = 1;
    elseif ~options.session.tvsys.saved &&(options.session.tvsys.filtered ||...
...
if ~errStat && ~strcmp(get(scr,'tag'),'out')
    ...
    id = []; type = [];
    auxVar = options.session.tvsys.varString.(get(scr,'tag')){...
        get(scr,'value')};
    if ~isempty(strfind(auxVar,'RRI'))
        id = 'ecg'; type = 'rri'; 
    elseif ~isempty(strfind(auxVar,'HR'))
...
if ~errStat
    options.session.tvsys.varValue.(get(scr,'tag')) = newValue;
    ...
    for i = 1:3
        if ~strcmp(get(scr,'tag'),opt{i})
            prevVar.(opt{i}) = options.session.tvsys.varString.(...
                get(puVar.(opt{i}),'tag')){...
                options.session.tvsys.varValue.(get(puVar.(opt{i}),'tag'))};
        end
    end
    otherPus = fieldnames(prevVar);
    
    oldVar = options.session.tvsys.varString.(get(scr,'tag')){oldValue};
    newVar = options.session.tvsys.varString.(get(scr,'tag')){newValue};
    ...
    if ~ismember(oldVar,{'No data available','Output variable',...
        ...
        for i = 1:length(otherPus)
            auxString = options.session.tvsys.varString.(otherPus{i});
            if ~isempty(strfind(oldVar,'DBP'))
                index(i) = length(options.session.tvsys.varString.(...
                    otherPus{i}))+1;
            else
                index(i) = 2;
            ...
            options.session.tvsys.varString.(otherPus{i}) = auxString;
        end
    end
    ...
    for i = 1:length(otherPus)
        ...
        options.session.tvsys.varString.(otherPus{i}) = auxString;
        options.session.tvsys.varValue.(otherPus{i}) = newInd;
...
if ~isempty(options.session.tvsys.sigLen)
    ...
    type = options.session.tvsys.sigSpec;
    for i = 1:length(type)
        if strcmp(type{i},'hr')
            type{i} = 'rri';
        end
...
if ~isempty(options.session.tvsys.sigLen) 
    ...
    if ~options.session.tvsys.filtered || ~options.session.tvsys.detrended
        if ~options.session.tvsys.filtered
            filtString = '\n Low-pass Kaiser filter';
        end
        if ~options.session.tvsys.detrended
    ...
    if ~errStat        
        ...
        tam = round((options.session.tvsys.perc/100)*length(curVar.output)); 
        ...
        if length(inName) == 2
            noteString = [sysName,': Output (',varName.output,'); ',...
                ...
                num2str(options.session.tvsys.perc),'% of data for ',...
                'system estimation'];
        elseif length(inName) == 1
            noteString = [sysName,': Output (',varName.output,'); ',...
                'Input (',varName.input1,'); ',num2str(...
                options.session.tvsys.perc),'% of data for system ',...
                'estimation'];
        else
            noteString = [sysName,': Output (',varName.output,'); ',...
                num2str(options.session.tvsys.perc),'% of data for ',...
        ...
        if options.session.tvsys.detrended
            ...
            set(te,'Name',[sysName,' data trends'],'OutputName',...
                ...
                num2str(options.session.tvsys.poly)]);
        end
        if options.session.tvsys.perc ~= 100
            ...
            if options.session.tvsys.detrended
                ...
                set(tv,'Name',[sysName,' data trends'],'OutputName',...
                    ...
                    options.session.tvsys.poly)]);
                t = merge(te,tv);
            end
            z = merge(ze,zv);
        else
            z = ze;
            if options.session.tvsys.detrended, t = te; end
        end
        ...
        if ~isempty(prevSys)
            for i = 1:length(prevSys)
                if isequal(patient.tvsys.(prevSys{i}).data.y,z.y) && ...
                        isequal(patient.tvsys.(prevSys{i}).data.u,z.u)
                    saved = 1;
                end
            end
        end
        ...
        elseif options.session.tvsys.saved
            [selButton, dlgShow] = uigetpref('CRSIDLabPref',...
        ...
        if ~saved
            ...
            pSys.data = z; 
            if options.session.tvsys.detrended, pSys.trends = t; end
            patient.tvsys.(sysName) = pSys;
            options.session.tvsys.saved = 1;
            set(userOpt,'userData',options);
...
if strcmp(get(scr,'tag'),'perc') && options.session.tvsys.detrended
...
if ~isnan(A) && ~errStat
    ...
    if ~errStat
        ...
        options.session.tvsys.(get(scr,'tag')) = value;

        set(userOpt,'userData',options);
        set(scr,'string',num2str(value));

        if strcmp(get(scr,'tag'),'perc') && ...
                ~isempty(options.session.tvsys.sigLen)
            plotFcn(pHandle,pCurVar,userOpt);
        end
    end
end
set(scr,'string',num2str(options.session.tvsys.(get(scr,'tag'))));
end
...
if ~isempty(options.session.tvsys.sigLen) 
    if strcmp(get(scr,'tag'),'filt') 
        if options.session.tvsys.filtered
            ...
            options.session.tvsys.viewFilter = 1;
        end
    elseif strcmp(get(scr,'tag'),'det')
        if options.session.tvsys.detrended
            ...
            options.session.tvsys.viewDetrend = 1;
        end
    end
    set(userOpt,'userData',options);
...
if ~errStat 
    ...
    options.session.tvsys.viewFilter = 0;
    options.session.tvsys.filtered = 1;
...
if ~isempty(options.session.tvsys.sigLen) && ~errStat
    ...
    tam = round((options.session.tvsys.perc/100)*length(t));
    noLines = 0;
    count = 1;
...
if options.session.tvsys.detrended
    uiwait(errordlg(['Signals already detrended! The signals have ',...
        ...
        'Signals already detrended','modal'));
    errStat = 1;
elseif options.session.tvsys.viewFilter
    ...
    for i = 1:3
        if ~isempty(var.(opt{i}))
            [var.(opt{i}),var.(trend{i})] = identDetrend(var.(opt{i}),...
                var.fs,options.session.tvsys.perc,options.session.tvsys.poly);
        end
    end
    
    options.session.tvsys.viewDetrend = 0;
    options.session.tvsys.detrended = 1;
...
options = get(userOpt,'userData');
options.tvsys.perc = options.session.tvsys.perc;
options.tvsys.poly = options.session.tvsys.poly;
set(userOpt,'userData',options);
end