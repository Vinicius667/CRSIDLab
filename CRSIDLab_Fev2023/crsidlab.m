...
pInfo = uicontrol('visible','off','userData',info);
% -------------------------------------------------------------------------
pTf = uicontrol('visible','off','userData',tf);
% -------------------------------------------------------------------------
...
options.session.ft.varString.in = '';
options.session.ft.varString.out = '';
options.session.ft.varValue.in = 1;
options.session.ft.varValue.out = 1;
options.session.ft.vlf = options.ft.vlf;
options.session.ft.lf = options.ft.lf;
options.session.ft.hf = options.ft.hf;
options.session.ft.minF = options.ft.minF;
options.session.ft.maxF = options.ft.maxF;
options.session.ft.coherence = options.ft.coherence;
options.session.ft.overlapping = options.ft.overlapping;
options.session.ft.window = options.ft.window;
options.session.ft.scale = options.ft.scale;
...
options.session.ft.sigLen = [];
options.session.ft.sigSpec = [];
% ---------------*---------------*---------------*--------------- 

% Time Variant PSD analysis
options.session.tvpsd.saved = 0;
options.session.tvpsd.exp = 0;
options.session.tvpsd.varString = '';
options.session.tvpsd.varValue = 1;
options.session.tvpsd.vlf = options.tvpsd.vlf;
options.session.tvpsd.lf = options.tvpsd.lf;
options.session.tvpsd.hf = options.tvpsd.hf;
options.session.tvpsd.minP = options.tvpsd.minP;
options.session.tvpsd.maxP = options.tvpsd.maxP;
options.session.tvpsd.minF = options.tvpsd.minF;
options.session.tvpsd.maxF = options.tvpsd.maxF;
options.session.tvpsd.arOrder = options.tvpsd.arOrder;
options.session.tvpsd.N = options.tvpsd.N;
options.session.tvpsd.cbSelection = options.tvpsd.cbSelection;
options.session.tvpsd.cbVisSelection = options.tvpsd.cbVisSelection;
options.session.tvpsd.rbWindow = options.tvpsd.rbWindow;
options.session.tvpsd.rbScale = options.tvpsd.rbScale;
options.session.tvpsd.segments = options.tvpsd.segments;
options.session.tvpsd.overlap = options.tvpsd.overlap;
options.session.tvpsd.fill = options.tvpsd.fill;
options.session.tvpsd.areas = struct;
...
% create time varying new system
options.session.tvsys.saved = 0;
options.session.tvsys.varString.out = '';
options.session.tvsys.varString.in1 = '';
options.session.tvsys.varString.in2 = '';
options.session.tvsys.varValue.out = 1;
options.session.tvsys.varValue.in1 = 1;
options.session.tvsys.varValue.in2 = 1;
options.session.tvsys.perc = options.tvsys.perc;
options.session.tvsys.poly = options.tvsys.poly;
...
options.session.tvident.sysKey = [];
options.session.tvident.model = options.tvident.model;
options.session.tvident.criteria = options.tvident.criteria;
options.session.tvident.orMax = options.tvident.orMax;
options.session.tvident.param = options.tvident.param;
options.session.tvident.gen = options.tvident.gen;
options.session.tvident.pole = options.tvident.pole;
options.session.tvident.sysMem = options.tvident.sysMem;
options.session.tvident.sysLen = [0 0 0];
options.session.tvident.freq = options.tvident.freq;
options.session.tvident.cbVis = [1,0,0,0,0,0,0];
...
set(tbMain,'callback',{@tabChange,tbMain,pnMain,tbPre,pnPre,tbAn,pnAn,...
    pFile,pTemp,pSig,pSys,pMod,pTf,pInfo,userOpt,dataList,addedData});
set(tbPre,'callback',{@tabChange,tbMain,pnMain,tbPre,pnPre,tbAn,pnAn,...
    pFile,pTemp,pSig,pSys,pMod,pTf,pInfo,userOpt});
...
tabChange(tbMain,[],tbMain,pnMain,tbPre,pnPre,tbAn,pnAn,pFile,pTemp,...
    pSig,pSys,pMod,pTf,pInfo,userOpt,dataList,addedData);

end

function tabChange(scr,~,tbMain,pnMain,tbPre,pnPre,tbAn,pnAn,pFile,...
    pTemp,pSig,pSys,pMod,pTf,pInfo,userOpt,dataList,addedData)
                ...
                case 2                      % TVPSD
                    if ~isempty(options.session.tvpsd.sigLen)
                        ...
                        if ~options.session.tvpsd.saved && ...
                                ((options.session.tvpsd.cbSelection(1) && ...
                                ~isequal(pat.sig.(id).(...
                                type).aligned.tvpsd.psdFFT,signals.(id).(...
                                type).aligned.tvpsd.psdFFT)) || ...
                                (options.session.tvpsd.cbSelection(2) && ...
                                ~isequal(pat.sig.(id).(...
                                type).aligned.tvpsd.psdAR,signals.(id).(...
                                type).aligned.tvpsd.psdAR)) || ...
                                (options.session.tvpsd.cbSelection(1) && ...
                                ~isequal(pat.sig.(id).(...
                                type).aligned.tvpsd.psdWelch,...
                                signals.(id).(type).aligned.tvpsd.psdWelch)))
                            dispDialog = 1;
                        end
                    end
                case 3                      % TV system identification
                        ...
                        case 1              % TV model estimation
                            ...
                            if options.session.tvident.sysLen(1) ~= 0
                                ...
                                sysName = options.session.tvident.sysKey{...
                                    options.session.tvident.sysValue-1};
                                prevMod = fieldnames(patient.tvsys.(...
                                    sysName).models);
                                if ~isempty(prevMod)
                                    for i = 1:length(prevMod)
...
if get(scr,'value') == 1 && ~errStat
    ...
    switch get(scr,'tag')
        case 'main'                         % main tab
            ...
            mainTab(pnMain,pFile,pTemp,pSig,pSys,pMod,pInfo,userOpt,...
        ...
        case 'an'                           % analysis tab
            ...
            set(tbPre,'value',0,'backgroundcolor',[.94 .94 .94]);
            set(pnPre,'visible','off'); delete(get(pnPre,'children'));
            analysisTab(pnAn,pFile,pSig,pSys,pMod,pTf,userOpt);
            set(pnAn,'visible','on');
    end
else
    if get(scr,'value') == 1
